...
const MTOSquare = ({ onClose, order }) => {
  ...
  const downloadPDF = () => {
    ...
    doc.html(contentRef.current, {
        callback: (doc) => doc.save(`MTOSalesOrder_${order.mto_order_number}.pdf`),
        x: 10,
        y: 10,
        width: 500,
    });
    ...
    >
      ...
      >
        <div style={{ display: "flex", justifyContent: "flex-end" }}>
          <button
            onClick={onClose}
            style={{ color: "red", fontSize: "14px", border: "none", background: "none" }}
          >
            Close
          </button>
        </div>
        ...
        >
          ...
          <div className="grid grid-cols-2">
            ...
            <div className="w-full text-right">
                ...
                <h2 style={{ fontSize: "14pt", fontWeight: "bold" }}>
                    Order #<u>{order.mto_order_number}</u>
                </h2>
            </div>
          ...
          >
            <h1 style={{ fontSize: "24pt", margin: 0 }}>{order.courier.name}</h1>
          </div>

        

          {/* Customer and Status Details */}
          <div className="border border-black flex flex-col mb-[15px] p-4">
            <div style={{ display: "flex", justifyContent: "space-between" }}>

                <h2 style={{ fontSize: "14pt", fontWeight: "bold" }}>
                    Customer: <u>{order.customers.first_name} {order.customers.last_name}</u>
                </h2>
                <h2 style={{ fontSize: "14pt", fontWeight: "bold" }}>Status: <u>{order.status}</u></h2>
            </div>
            <div  style={{ display: "flex", justifyContent: "space-between" }}>
                <h2 style={{ fontSize: "14pt", fontWeight: "bold" }}>
          ...
          >
            <h1 style={{ fontSize: "20pt", margin: 0 }}>ORDER DETAILS</h1>
          </div>
          <hr style={{ margin: "12px 0", border: "1px solid #000" }} />

          <table className="border border-black" style={{ width: "100%", borderCollapse: "collapse", marginBottom: "20px" }}>
            <thead>
              <tr>
                <th style={{ borderBottom: "1px solid #000", padding: "8px", textAlign: "left" }}>
                  Items
                </th>
                <th style={{ borderBottom: "1px solid #000", padding: "8px", textAlign: "center" }}>
                  Qty
                </th>
              </tr>
            </thead>
            <tbody>
              {order.items.map((item, index) => (
                <tr key={index}>
                  <td style={{ padding: "8px", borderBottom: "1px solid #000" }}>
                    {item.made_to_order_product.product_name} {' '}
                    {item.made_to_order_product.color} 
                    {item.made_to_order_product.size &&
                        ` S: ${item.made_to_order_product.size} `}
                    {item.made_to_order_product.heel_height &&
                        ` HH: ${item.made_to_order_product.heel_height} `}
                    {item.made_to_order_product.type_of_heel &&
                        ` TYPE: ${item.made_to_order_product.type_of_heel}  `}
                    {item.made_to_order_product.round &&
                        ` RND: ${item.made_to_order_product.round}  `}
                    {item.made_to_order_product.length &&
                        ` LNGTH: ${item.made_to_order_product.length}  `}
                    {item.made_to_order_product.back_strap &&
                        ` BCKSTRP: ${item.made_to_order_product.back_strap}`}
                  </td>
                  <td
                    style={{
                      padding: "8px",
