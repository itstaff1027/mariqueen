...
const MTOOrderEdit = ({ made_to_order }) => {
    ...
    const allowedStatuses = (user) => {
        ...
        const statuses = [];
        if (allPermissions.some(perm => perm.name === 'update' && made_to_order.status !== 'cancelled' && made_to_order.status !== 'shipped' && made_to_order.status !== 'delivered' && made_to_order.status !== 'approved')) {
            statuses.push('approved');
        }
        if (allPermissions.some(perm => perm.name === 'cancel' && made_to_order.status !== 'cancelled')) {
    ...
    const updateStatus = (status) => {
        if (
            ...
            router.post(`/made_to_orders/update/status/${made_to_order.id}`, { new_status: newStatus})
